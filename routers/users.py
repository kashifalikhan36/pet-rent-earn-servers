...
async def get_user_dashboard_stats(
    ...
    try:
        # Get pet listings count
        pet_count = await database.pets.count_documents({"owner_id": user_id})
        active_pet_count = await database.pets.count_documents({
            "owner_id": user_id, 
            "status": "active"
        })
        ...
        favorite_count = len(favorites.get("pet_ids", [])) if favorites else 0
        
        # TODO: Add transaction stats, earnings, etc. when those collections are implemented
        
        stats = {
            "total_listings": pet_count,
            "active_listings": active_pet_count,
            "total_favorites": favorite_count,
            "total_bookings": 0,  # TODO: Implement when transactions are added
